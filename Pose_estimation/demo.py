...
def main():
    ...
    with torch.no_grad():
        ...
        input_path ='' # insert the directory of mp4 files    './test_videos'
        paths = []
        names = []
        ...
        output_dir ='' # insert the directory where the keypoint .npy files will be saved npy     './wholepose_test_npy' 
        os.makedirs(output_dir, exist_ok=True) 
        
        for i, path in enumerate(paths):
            # if i > 1:
            #     break
            video_name = os.path.splitext(names[i])[0]  # removing .mp4 part
            #output_npy = f'/home/smanjur/ICCV/data/wholepose_test_npy/{video_name}.npy'
            output_npy=f'{output_dir}/{video_name}.npy'
            ...
            while cap.isOpened():
                ...
                if not success:
                    ...
                    break
                frame_count += 1
                img = cv2.resize(img, (512,512)) # i uncommented it
