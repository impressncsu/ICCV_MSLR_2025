...
def gendata(data_path, label_path, out_path, part='train', config='27'):
    ...
    if label_path is None or label_path == 'None':
        print("No label file provided — processing all .npy files in directory.")
        npy_files = sorted([f for f in os.listdir(data_path) if f.endswith('.npy')])
        
        for f in npy_files:
            name = f.replace('.npy','')
            sample_names.append(name)
            data.append(os.path.join(data_path, f))
    ...
    for line in label_file.readlines():
        ...
        sample_names.append(line[0])
        data.append(os.path.join(data_path, line[0] + '_color.npy')) # removed .mp4
        # print(line[1])
        labels.append(int(line[1]))
        # print(labels[-1])
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='../wholepose_test_npy') #'train_npy/npy', 'va_npy/npy'
    ...
    parser.add_argument('--out_folder', default='../data/sign/27/testing_data')
